...
def PrepareImageData(ImgData, StartX, StartY, EndX, EndY):
    ...
    for Y in range(StartY, EndY):
        for X in range(StartX, EndX):
            #Pixel = ImgData[X, Y];
            Output[X - StartX][Y - StartY] = ImgData[X, Y]#(Pixel[0]) + ((Pixel[1]) << 8) + ((Pixel[2]) << 16);
    return Output;
...
for File in Images:
    ...
    Left = 220#(SizeRaw[0] * 1/3);
    Right = 1350#(SizeRaw[0] * 2/3);
    Top = 150#(SizeRaw[1] * 1/3);
    Bottom = 675#(SizeRaw[1] * 2/3);
