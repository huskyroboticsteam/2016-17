...
class Robot(object):
    ...
    def __init__(self, is_using_big_motor, coord_list):
        """
        Args:
        ...
        if is_using_big_motor == "0":
            ...
            self.nav = Navigation.Navigation(0.51555, 0.33277, 0.14611, 0.001, "AIN2")
    ...
    def getDriveParms(self):
        ...
        if len(self.coord_list) > 0:
            ...
            if self.autonomous.is_done(location):
                # Reached the target
                # set target to the next coord
                #self.r_comms.sendAtLocationPacket(self.nav)
                self.coords_list = self.coords_list[1:len(self.coords_list)]
                self.autonomous_initialized = False
                return 0, 0
            else:
            ...
            print("Arrived at desired location: " + str(self.target))
            return 0, 0
